...
@Named("orderedProductController")
@SessionScoped
public class OrderedProductController implements Serializable {
    ...
    @EJB
    private JSF.OrderedProductFacade ejbFacade;
    ...
    public OrderedProduct getSelected() {
        if (current == null) {
            ...
            current.setOrderedProductPK(new Entity.OrderedProductPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getOrderedProductPK().setCustomerId(current.getUser().getId());
            current.getOrderedProductPK().setProductId(current.getProduct().getId());
            getFacade().create(current);
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("ressources/messages").getString("OrderedProductCreated"));
    ...
    public String update() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("ressources/messages").getString("OrderedProductUpdated"));
            return "View";
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("ressources/messages").getString("PersistenceErrorOccured"));
            return null;
    ...
    private void performDestroy() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("ressources/messages").getString("OrderedProductDeleted"));
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("ressources/messages").getString("PersistenceErrorOccured"));
        }
    }
    ...
    public OrderedProduct getOrderedProduct(Entity.OrderedProductPK id) {
    ...
    public static class OrderedProductControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            return controller.getOrderedProduct(getKey(value));
        }

        Entity.OrderedProductPK getKey(String value) {
            Entity.OrderedProductPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new Entity.OrderedProductPK();
            key.setProductId(Integer.parseInt(values[0]));
            key.setCustomerId(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(Entity.OrderedProductPK value) {
