...
<body class="common-home">
	...
	<div class="container">
		...
		<h3><fmt:message bundle="${bundle}"  key="search.featured" /></h3>
		<div class="row product-layout">
                    <c:choose>
                        <c:when test="${empty product}">
                            <fmt:message bundle="${bundle}"  key="search.productNotFound" />
                        </c:when>
                        <c:otherwise>
				...
				<div class="product-thumb transition">
					...
					<div class="button-group">
						<button style="width: 100%" type="button" onclick="cart.add('${bean.id}');"><i class="fa fa-shopping-cart"></i> <span class="hidden-xs hidden-sm hidden-md"><fmt:message bundle="${bundle}" key="search.addToCart" /></span></button>
					</div>
				</div>
			</div>
				</c:forEach>
